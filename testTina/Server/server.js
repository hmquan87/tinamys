...
app.get("/getDataCompany", async (req, res) => {
    ...
    try {
        ...
        const parsedData = JSON.parse(data);
        if (id) {
            const companySpace = parsedData.companySpace.find(company => company.id === parseInt(id));

            if (!companySpace) {
                return res.status(404).json({ error: "Company not found" });
            }

            res.json({ success: true, companySpace });
        } else {
            res.json({ success: true, companySpace: parsedData.companySpace });
        }
    } catch (err) {
